...
##    img_list = ['barbara.png']
    for img_test in img_list:
        ...
        for m in ms:
            ...
            pth = os.path.join('.','out_inpainting_old_method',img_test[:-4],'m%d'%m)
            ...
            for l in range(len(lams)):
                ...
                for j in range(N):
                    Phi = Psi[j][range(m)]
                    Vhi = Psi[j][np.random.randint(1,n)]
                    ...
                    Verr0[l] += np.power(Vhi.dot(X[:,j]-xj),2)
...
##                print img_test, m, lams[l], PSNR0[l], Verr0[l]
                ...
                for i in range(T):
                    ...
                    for j in range(N):
                        ...
                        A.append(aj)
                        xj = D.dot(aj.toarray())
                        Grad += (Phi.T).dot(yj - Phi.dot(xj)).dot(aj.toarray().T)
                    ...
                    for j in range(N):
                        ...
                        Vhi = Psi[j][np.random.randint(1,n)]
                        yj = Phi.dot(X[:,j])
                        xj = D.dot(A[j].toarray()).squeeze()
                        eps[j] = np.sum(np.power(yj - Phi.dot(xj),2))
                        PSNR[i,l] += np.power(X[:,j]-xj,2).mean()
                        Verr[i,l] += np.power(Vhi.dot(X[:,j]-xj),2)
                    eps *= k
